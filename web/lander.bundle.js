...
function renderExtension(mode2) {
  el("enc-key").textContent = identity2.publicKey;
  ...
  if (mode2 === "extension") {
    ...
    el("extension-hint").textContent = "Your key is held in this browser for now \u2014 fine for receiving. Install the extension for maximum privacy, or to send.";
  }
  ...
  checkTimer = setTimeout(async () => {
    try {
      ...
      if (result.available) {
        ...
        el("claim").disabled = !(identity2 && wallet);
      } else {
...
async function boot() {
  ...
  const [health, mode2, circle] = await Promise.all([
  ...
  if (identity2) {
    name = await gateway.reverseName(identity2.publicKey);
  }
  renderName();
}
