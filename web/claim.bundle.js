...
var Gateway = class {
  ...
  async #authFields(identity2) {
    const pubKey = toHex(identity2.publicKey);
    const { nonce } = await this.#json("/api/nonce", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ pubKey })
    });
    const signature = toHex(sign(sha2562(new TextEncoder().encode(nonce)), identity2.privateKey));
    return { pubKey, nonce, signature };
  }
  health() {
    return this.#json("/api/health");
  }
  async send(identity2, payload) {
    return this.#json("/api/send", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...await this.#authFields(identity2), ...payload })
    });
  }
  transfer(id2) {
  ...
  async claim(identity2, id2, claimSignature) {
    return this.#json("/api/claim", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...await this.#authFields(identity2), id: id2, claimSignature })
    });
  }
};
...
async function load() {
  ...
  if (transfer.recipientPubKey !== identity.publicKeyHex) {
    el("identity-hint").textContent = "This file was sent to a different key. Give the sender the key above and ask them to send it again.";
