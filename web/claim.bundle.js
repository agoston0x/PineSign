...
var Gateway = class {
  ...
  transfer(id2) {
    return this.#json(`/api/transfer/${id2}`);
  }
  async blob(id2) {
    const res = await fetch(`${this.baseUrl}/api/blob/${id2}`);
    if (!res.ok) throw new Error(`blob fetch failed: ${res.status}`);
    return new Uint8Array(await res.arrayBuffer());
  }
  async claim(signer2, id2, claimSignature) {
    return this.#post("/api/claim", { ...await this.#auth(signer2), id: id2, claimSignature });
  }
  // ---- names ----
...
// shared/keysource.js
var mode = null;
async function keyMode() {
  if (mode) return mode;
  mode = await extensionPresent() ? "extension" : "page";
  return mode;
}
async function identity() {
  if (await keyMode() === "extension") return getIdentity();
  const id2 = await getOrCreateIdentity();
  return { publicKey: id2.publicKeyHex, address: id2.address };
}
async function signer() {
  if (await keyMode() === "extension") return bridgeSigner();
  return localSigner(await getOrCreateIdentity());
}
async function openTransfer2({ ciphertext, senderPubKey, expectedHash }) {
  if (await keyMode() === "extension") {
    return openTransfer({ ciphertext: Array.from(ciphertext), senderPubKey, expectedHash });
  }
  const id2 = await getOrCreateIdentity();
  const sharedKey = deriveSharedKey(id2.privateKey, fromHex(senderPubKey));
  ...
  const digest = transferDigest({ senderPubKey, recipientPubKey: id2.publicKeyHex, plaintextHash: hash });
  return { plaintext: Array.from(plaintext), plaintextHash: hash, signature: toHex(sign(digest, id2.privateKey)) };
}
...
var short = (hex) => `${hex.slice(0, 10)}\u2026${hex.slice(-6)}`;
...
async function load() {
  ...
  const mode2 = await keyMode();
  identity2 = await identity();
  const named = await gateway.reverseName(identity2.publicKey);
  el("pubkey").textContent = named ? named.name : identity2.publicKey;
  ...
  if (transfer.requireExtension && mode2 !== "extension") {
  ...
  el("key-mode").textContent = mode2 === "extension" ? "Your key is in the extension. Nothing this page loads can reach it." : "Your key is held in this browser. Private in use \u2014 but you are trusting the code this page served you.";
  if (transfer.recipientPubKey !== identity2.publicKey) {
    el("identity-hint").textContent = "This file was sent to a different key. Give the sender the name above and ask them to send it again.";
...
el("accept").addEventListener("click", async () => {
  ...
  try {
    ...
    const opened = await openTransfer2({
